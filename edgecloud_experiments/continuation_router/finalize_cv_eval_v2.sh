...
REPORT=edgecloud_experiments/reports/CONTINUATION_VERIFIED_ROUTER_RESULTS_V2_20260512_ZH.md

echo "[watch] waiting for v2 continuation-router eval processes"
while pgrep -af "eval_cv_edgecloud_r2r.py.*cv_eval_val_unseen_v2" >/dev/null; do
  sleep 120
done

if [[ ! -d "$OUT_ROOT" ]]; then
  echo "No v2 eval directory found: $OUT_ROOT" >&2
  exit 0
fi
...
report = Path("edgecloud_experiments/reports/CONTINUATION_VERIFIED_ROUTER_RESULTS_V2_20260512_ZH.md")
report.parent.mkdir(parents=True, exist_ok=True)
lines = [
    "# Continuation-Verified Router v2 Full Eval Results",
    "",
    "该文件由 `finalize_cv_eval_v2.sh` 自动生成。",
    "",
