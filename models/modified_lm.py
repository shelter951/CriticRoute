    ...
    def forward(
        ...
        **kwargs
        ...
        if cand_locations.sum() != 0 and cand_vis is not None:
            ...
            elif isinstance(cand_vis, (list, tuple)) and len(cand_vis) > 0:
                if isinstance(cand_vis[0], torch.Tensor):
                    cand_vis = torch.stack([t.to(device=device, dtype=dtype) for t in cand_vis], dim=0)
                else:
                    cand_vis = None
        ...
        if obj_locations.sum() != 0 and obj_vis is not None:
            ...
            elif isinstance(obj_vis, (list, tuple)) and len(obj_vis) > 0:
                if isinstance(obj_vis[0], torch.Tensor):
                    obj_vis = torch.stack([t.to(device=device, dtype=dtype) for t in obj_vis], dim=0)
                else:
                    obj_vis = None
