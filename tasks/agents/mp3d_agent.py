...
import sys
import os
# 添加tools目录到路径，以便导入DistillLogger
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))
try:
    from tools.distill_logger import DistillLogger
except ImportError:
    DistillLogger = None
...
class MP3DAgent(BaseAgent):
    ...
    def _calculate_distance_to_goal(self, obs_i: Dict, goal_vp: Optional[str] = None) -> float:
        """
        计算当前viewpoint到目标viewpoint的距离
        
        Args:
            obs_i: 单个observation字典
            goal_vp: 目标viewpoint ID，如果为None则使用gt_path[-1]
        
        Returns:
            距离（float），如果无法计算则返回-1.0
        """
        try:
            scan = obs_i['scan']
            current_vp = obs_i['viewpoint']
            
            if goal_vp is None:
                if 'gt_path' in obs_i and len(obs_i['gt_path']) > 0:
                    goal_vp = obs_i['gt_path'][-1]
                else:
                    return -1.0
            
            if goal_vp not in self.shortest_distances[scan][current_vp]:
                return -1.0
                
            return float(self.shortest_distances[scan][current_vp][goal_vp])
    ...
    def rollout(
        ...
        **kwargs
        ...
        # ========== 初始化蒸馏数据收集 ==========
        # 检查是否启用蒸馏日志（通过args.enable_distill_log控制）
        enable_distill_log = getattr(args, 'enable_distill_log', False) and validate
        distill_logger = None
        distill_episode_data = []  # 存储每个episode的数据
        
        if enable_distill_log and DistillLogger is not None:
            # 获取split信息（从dataset或args中）
            split = getattr(dataset, 'split', getattr(args, 'validation_split', 'unknown'))
            output_dir = getattr(args, 'distill_output_dir', os.path.join(args.output_dir, 'distill_logs'))
            distill_logger = DistillLogger(output_dir, name, split, enabled=True)
            
            # 为每个episode初始化数据收集结构
            for i in range(batch_size):
                distill_episode_data.append({
                    'task': name,
                    'split': split,
                    'episode_id': obs[i]['instr_id'],
                    'steps': []
                })
        # ==========================================
        ...
        for t in range(max_action_len):
            ...
            with context():
                ...
                # ========== 收集蒸馏数据：记录prompt和logits ==========
                if enable_distill_log:
                    # 计算dist_before（执行动作前的距离）
                    dist_before_list = []
                    for i in range(batch_size):
                        if not ended[i]:
                            dist_before = self._calculate_distance_to_goal(obs[i])
                            dist_before_list.append(dist_before)
                        else:
                            dist_before_list.append(-1.0)
                # ========================================================
                ...
                if enable_distill_log:
                    ...
                    for i in range(batch_size):
                        if not ended[i]:
                            # 获取当前step的候选viewpoint IDs（包含stop）
                            # nav_vpids[i][0] 是 None (stop)，nav_vpids[i][1:] 是其他候选
                            cand_vpids_list = [None]  # stop在位置0
                            valid_indices = [0]  # stop的索引
                            
                            for j in range(1, len(nav_vpids[i])):
                                if cand_masks[i][j]:
                                    cand_vpids_list.append(nav_vpids[i][j])
                                    valid_indices.append(j)
                            
                            # 获取logits（只取有效候选的，顺序与cand_vpids_list对应）
                            valid_logits = []
                            for idx in valid_indices:
                                valid_logits.append(float(nav_logits[i][idx].cpu().item()))
                            
                            # 获取action信息
                            action_idx_global = int(a_t[i].cpu().item()) if a_t[i] != -100 else 0
                            # 将全局索引转换为cand_vpids_list中的局部索引
                            action_idx_local = -1
                            if action_idx_global in valid_indices:
                                action_idx_local = valid_indices.index(action_idx_global)
                            
                            action_vpid = None
                            if action_idx_global == 0:
                                action_vpid = None  # stop
                            elif action_idx_global < len(nav_vpids[i]):
                                action_vpid = nav_vpids[i][action_idx_global]
                            
                            # 记录step数据（先不记录dist_after，等env.step后再更新）
                            step_data = {
                                't': t,
                                'viewpoint_id_before': obs[i]['viewpoint'],
                                'scan': obs[i].get('scan', ''),  # 添加scan信息用于加载视觉特征
                                'schema_prompt': nav_inputs["prompts"][i],
                                'cand_vpids': cand_vpids_list,
                                'teacher_logits': valid_logits,
                                'teacher_action_idx': action_idx_local,  # 使用局部索引
                                'teacher_action_idx_global': action_idx_global,  # 保留全局索引用于调试
                                'teacher_action_vpid': action_vpid,
                                'dist_before': dist_before_list[i],
                                'dist_after': -1.0,  # 稍后更新
                                'done': 0
                            }
                            distill_episode_data[i]['steps'].append(step_data)
                # ========================================================
                ...
                if enable_distill_log:
                    for i in range(batch_size):
                        if not ended[i] and len(distill_episode_data[i]['steps']) > 0:
                            # 更新最后一个step的dist_after
                            last_step = distill_episode_data[i]['steps'][-1]
                            dist_after = self._calculate_distance_to_goal(obs[i])
                            last_step['dist_after'] = dist_after
                            last_step['done'] = 1 if ended[i] else 0
                # ========================================================

                if flag:
                    break
        
        # ========== 蒸馏数据：计算episode metrics并写入日志 ==========
        if enable_distill_log and distill_logger is not None:
            # 保存初始obs用于计算metrics
            initial_obs = batch_dict["observations"]
            
            for i in range(batch_size):
                if len(distill_episode_data[i]['steps']) == 0:
                    continue
                
                # 计算episode的success和spl
                # 使用dataset的eval_dis_item或eval_cvdn方法计算metrics
                try:
                    # 获取预测路径和ground truth路径
                    pred_path = traj[i]['path']
                    # 从初始obs获取gt_path
                    gt_path = initial_obs[i].get('gt_path', None)
                    
                    if gt_path is not None:
                        scan = initial_obs[i]['scan']
                        scores = {}
                        
                        # 尝试不同的eval方法（不同任务有不同的方法）
                        if hasattr(dataset, 'eval_cvdn') and name == 'CVDN':
                            # CVDN使用eval_cvdn方法，需要gt_item
                            try:
                                # 从batch_dict中获取gt_item信息
                                item = batch_dict['item'][i]
                                gt_item = dataset.gt_trajs.get(initial_obs[i]['instr_id'], {})
                                if gt_item:
                                    path_flat = sum(pred_path, [])
                                    scores = dataset.eval_cvdn(scan, path_flat, gt_item)
                            except:
                                pass
                        elif hasattr(dataset, 'eval_dis_item'):
                            # 对于R2R/EQA等任务
                            try:
                                scores = dataset.eval_dis_item(scan, pred_path, gt_path)
                            except TypeError:
                                # 对于REVERIE等需要额外参数的任务
                                try:
                                    pred_objid = traj[i].get('pred_objid', None)
                                    gt_objid = initial_obs[i].get('gt_obj_id', None)
                                    scores = dataset.eval_dis_item(scan, pred_path, pred_objid, gt_path, gt_objid)
                                except:
                                    scores = {}
                        
                        success = int(scores.get('success', 0))
                    ...
                    # 如果计算失败，使用默认值
                    if args.rank == 0:  # 只在主进程打印警告
                        print(f"Warning: Failed to calculate metrics for episode {distill_episode_data[i]['episode_id']}: {e}")
                    success = 0
                    spl = 0.0
                
                # 更新episode数据
                distill_episode_data[i]['success'] = success
                distill_episode_data[i]['final_sr'] = success
                distill_episode_data[i]['final_spl'] = spl
                
                # 写入日志
                distill_logger.log_episode(distill_episode_data[i])
            
            # 关闭logger
            distill_logger.close()
        # ============================================================
