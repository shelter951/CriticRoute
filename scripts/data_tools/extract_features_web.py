...
def process_features(proc_id, out_queue, scanvp_list, args):
    if args.eva_clip_repo:
        sys.path.append(args.eva_clip_repo)
    ...
    for i, d_meta in enumerate(scanvp_list):
        ...
        if aug_frame_temporal:
            try:
                image = Image.open(os.path.join(args.frame_root, vid, "imgs_3fps_360p", new_fid))
                fid = new_fid
            except:
                image = Image.open(os.path.join(args.frame_root, vid, "imgs_3fps_360p", fid))
        else:
            image = Image.open(os.path.join(args.frame_root, vid, "imgs_3fps_360p", new_fid))
        if aug_frame_spatial:
...
def main(args):
    ...
    for vid in anno_vids:
        included = []
        with open(os.path.join(args.anno_dir, '%s.json' % vid)) as f:
            data = json.load(f)
            for x in data:
                for fid in x['frames']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--anno_dir', default='data/RoomTour/object_progression_annotations')
    parser.add_argument('--frame_root', default='data/RoomTour/trajectory_scamera')
    parser.add_argument('--eva_clip_repo', default='EVA/EVA-CLIP/rei')
