...
def val_one_epoch(
    ...
    for name, loader in dataloaders.items():
        ...
        if args.rank == 0 and args.save_pred_results:
            # Object-navigation evaluators may require grounding fields that are
            # irrelevant to the edge-cloud router. Save raw trajectories before
            # metric-specific formatting so cache generation never loses a
            # completed rollout because of optional object fields.
            raw_pred_path = os.path.join(args.output_dir, f"{name}_{args.validation_split}_raw.json")
            with open(raw_pred_path, "w", encoding="utf-8") as f:
                json.dump(all_preds, f, ensure_ascii=False)
            if os.environ.get("NAVILLM_RAW_PRED_ONLY", "0") == "1":
                logger.info(f"Saved raw predictions to {raw_pred_path}; skip metric-specific eval/save_json.")
                continue
