...
ROOT=build/hetero_router/eval_val_unseen_full_controls_v2_gpu4_7
...
echo "START R2R full controls gpu4-7 $(date)"
while pgrep -af 'run_soon_clean_split_controls_v1.sh|eval_object_edgecloud_nav.py' | grep -q 'soon_router_clean_split_v2/eval_controls'; do
  echo "WAIT_SOON_CONTROLS $(date)"
  sleep 180
done
...
echo "DONE R2R full controls gpu4-7 $(date)"
